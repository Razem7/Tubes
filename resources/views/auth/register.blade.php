...
@section('title', 'GadgetHub - Daftar')
...
<div class="flex min-h-[calc(100vh-64px)] items-center justify-center bg-slate-50 px-4 py-10">
    <div class="w-full max-w-sm">
        <div class="mb-8 text-center">
            ...
            <p class="mt-2 text-sm text-slate-500">Buat akun dan mulai berjualan 🚀</p>
        </div>
        ...
        <div class="rounded-[28px] border border-slate-200 bg-white/90 p-8 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)] backdrop-blur-sm">
            <h1 class="mb-6 text-xl font-bold text-slate-900">Buat akun baru</h1>

            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="name" class="mb-1.5 block text-sm font-medium text-slate-700">Nama Lengkap</label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Abdul Saputra" class="input-field {{ $errors->has('name') ? 'border-rose-300 bg-rose-50 focus:border-rose-400 focus:ring-rose-100' : '' }}" required autocomplete="name">
                    @error('name')
                ...
                <div>
                    <label for="phone_number" class="mb-1.5 block text-sm font-medium text-slate-700">Nomor HP <span class="font-normal text-slate-400">(opsional)</span></label>
                    <input type="text" id="phone_number" name="phone_number" value="{{ old('phone_number') }}" placeholder="08xxxxxxxxxx" class="input-field">
                </div>

                <div>
                    <label for="password" class="mb-1.5 block text-sm font-medium text-slate-700">Password</label>
                    <div class="relative">
                        <input type="password" id="password" name="password" placeholder="Min. 8 karakter" class="input-field pr-10 {{ $errors->has('password') ? 'border-rose-300 bg-rose-50 focus:border-rose-400 focus:ring-rose-100' : '' }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" id="password_confirmation" name="password_confirmation" placeholder="Ulangi password" class="input-field pr-10" required>
                        <button type="button" onclick="togglePassword('password_confirmation','eyeIcon2')" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                ...
                <button type="submit" class="mt-2 w-full rounded-2xl bg-gradient-to-r from-blue-600 to-sky-500 py-3 text-sm font-semibold text-white transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                    Buat Akun
                </button>
            </form>
        ...
        <p class="mt-6 text-center text-xs text-slate-400">Dengan mendaftar, kamu menyetujui syarat & ketentuan GadgetHub.</p>
    </div>
</div>
